...
@foreach ($payments as $payment)
    <div class="modal fade" id="editPaymentModal-{{ $payment->id }}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="editPaymentModalLabel" aria-hidden="true">
                    ...
                    <form action="{{ route('payments.update', ['payment' => $payment->id]) }}" method="POST">
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-6">
                                ...
                                <input type="number" name="amount" id="amount" class="form-control" value="{{ $payment->amount }}" step="0.01" min="0" required>
                            </div>
                            <div class="col-sm-6">
                                <label for="type">Payment Type</label>
                                <select name="type" id="type" class="form-control">
                                    <option value="rent" {{ $payment->type == 'rent' ? 'selected' : '' }}>Rent</option>
                                    <option value="utility" {{ $payment->type == 'utility' ? 'selected' : '' }}>Utility</option>
                                    <option value="advance" {{ $payment->type == 'advance' ? 'selected' : '' }}>Advance</option>
                                </select>
                            </div>
                            <div class="col-sm-6">
                            ...
                            <div class="col-sm-6">
                                ...
                                <input type="number" name="year_paid" id="year_paid" class="form-control" value="{{ $payment->year_paid }}" min="1900" max="{{ date('Y') }}">
                            </div>
